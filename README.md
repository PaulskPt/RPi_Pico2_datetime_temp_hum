...
The script is running on a Raspberry Pi Pico 2.
...
- aht.py by Jonatham Fromentin (Github etno712), changes by @PaulskPt
...
The font is a global font colection downloaded from Google Fonts: 
...
From this my_glbls varios variables are set.
Then it creates instances of:
...
Shell (or Serial Monitor) and display them on the display.
Next the rtc is set for a 24hour clock and set "rtc.enable_periodic_update_interrupt".
Next the function main() takes control. in a sequence interval of 1 second 
...
The script also sets a euro character, variable name "euro", defined as: chr(8364) (0x20AC).
...
When the oscilloscope was switched off, and the oscilloscope probes were still connected to an I2C breakout socket (nr 2), the script crashed in file: "aht.py",
